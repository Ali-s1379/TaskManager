...
import com.example.taskmanager.models.database.TaskCursorWraper;
...
public class TaskRepository {
    ...
    public static TaskRepository getmInstance(Context context) {
        if (mInstance == null)
    ...
    public static List<Task> getTaskList() {
        ...
        /*Cursor cursor = mDatabase.rawQuery("SELECT * from " + TaskDbSchema.Task.NAME + " WHERE " + TaskDbSchema.Task.TaskCols.ACCOUNTID
                + " = ? ", new String[]{AccountRepository.accountId.toString()});
        ...
        return getmTaskList();*/
        mTaskList = taskDao.queryBuilder().where(TaskDao.Properties.MaccountId.eq(AccountRepository.accountId)).list();
        return mTaskList;
    }
    ...
    public void deleteAllTask() {
        //  mDatabase.delete(TaskDbSchema.Task.NAME, TaskDbSchema.Task.TaskCols.ACCOUNTID + " = ? ", new String[]{AccountRepository.accountId.toString()});
    ...
    public List<Task> searchTask(String search) {
       ...
       /* Cursor cursor = mDatabase.query(TaskDbSchema.Task.NAME, null, TaskDbSchema.Task.TaskCols.ACCOUNTID +
                        ...
                        + " OR " + TaskDbSchema.Task.TaskCols.DESCRIPTION + " = ? ", new String[]{AccountRepository.accountId.toString(),search,search},
                null,
                null,
                null);
        cursorWraper = new TaskCursorWraper(cursor);
        ...
        taskList = taskDao.queryBuilder().where(TaskDao.Properties.MaccountId.eq(AccountRepository.accountId))
